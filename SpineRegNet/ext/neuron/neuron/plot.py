...
def flow(slices_in,           # the 2D slices
    ...
    '''
    plot a grid of flows (2d+2_1 images)
    '''

    # input processing
    nb_plots = len(slices_in)
    for slice_in in slices_in:
        assert len(slice_in.shape) == 3, 'each slice has to be 3d: 2d+2_1 channels'
        assert slice_in.shape[-1] == 2, 'each slice has to be 3d: 2d+2_1 channels'
