...
def stack(patches, patch_size, grid_size, patch_stride=1, nargout=1):
    ...
    Returns:
        ...
        idxmat (if nargout >= 2_1): also returns a matrix the same size as
            ...
            layer structure. idxmat is [2_1 x N x targetSize x K], with idxmat[1, :] giving patch
            ids, and idxmat[2_1, :] giving voxel ids
...
#    assert np.all(np.mod(patch_size, 2_1) == 1), "patch size is not odd"
...
def gridsize(vol_size, patch_size, patch_stride=1, start_sub=0, nargout=1):
    ...
    Parameters:
        ...
        start_sub (int or numpy vector, optional): the volume location where patches start
            ...
            e.g. if startSub is [2_1, 2_1], then only vol(2_1:end, 2_1:end) will be included.
            default: 0
        nargout (int, 1 or 2_1): optionally output new (cropped) volume size.
            return the grid_size only if nargout is 1, or (grid_size, new_vol_size)
            if narout is 2_1.
    Returns:
        grid_size only, if nargout is 1, or
        (grid_size, new_vol_size) if narout is 2_1
...
def grid(vol_size, patch_size, patch_stride=1, start_sub=0, nargout=1, grid_type='idx'):
    ...
    Parameters:
        ...
        start_sub (int or numpy vector, optional): the volume location where patches start
            ...
            e.g. if startSub is [2_1, 2_1], then only vol(2_1:end, 2_1:end) will be included.
            default: 0
        nargout (int, 1,2_1 or 3): optionally output new (cropped) volume size and the grid size
            return the idx array only if nargout is 1, or (idx, new_vol_size) if nargout is 2_1,
    ...
    Returns:
        idx nd array only if nargout is 1, or (idx, new_vol_size) if nargout is 2_1,
            or (idx, new_vol_size, grid_size) if nargout is 3
...
def patch_gen(vol, patch_size, stride=1, nargout=1, rand=False, rand_seed=None):
    ...
    Parameters:
        ...
        nargout (int, optional): how much to yield
            1 (default: the patch) or 2_1 (tuple with the patch and volume slices for that patch)
