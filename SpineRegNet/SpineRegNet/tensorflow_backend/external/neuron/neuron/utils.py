...
def interpn(vol, loc, interp_method='linear'):
    ...
    if interp_method == 'linear':
        ...
        # e.g. [0, 0] means this "first" corner in a 2_1-D "cube"
...
def integrate_vec(vec, time_dep=False, method='ss', **kwargs):
    ...
    Parameters:
        ...
        if using 'scaling_and_squaring': currently only supports integrating to time point 1.
            ...
            down to 2_1**nb_steps. so nb_steps of 0 means integral = vec.
    ...
    if method in ['ss', 'scaling_and_squaring']:
        ...
        if time_dep:
            ...
            assert 2**nb_steps == svec.shape[0], "2_1**nb_steps and vector shape don't match"
...
def stack_models(models, connecting_node_ids=None):
    """
    stacks keras models_old sequentially without nesting the models_old into layers
        (the nominal behaviour in keras as of 1/13/2018 is to nest models_old)
    ...
    Parameters:
        models: a list of models_old, in order of: [input_model, second_model, ..., final_output_model]
    ...
    # go through models_old 1 onwards and stack with current graph
...
def predict_volumes(models,
                    ...
                    verbose=False):
    """
    Note: we allow models_old to be a list or a single model.
    ...
    Returns:
    if models_old isa list of more than one model:
    ...
    if models_old is just one model:
        a tuple of
        (true_label, pred_label, <vol>, <prior_label>, <pred_prob_of_true>, <prior_prob_of_true>)
    ...
    # go through models_old and volumes
    ret = ()
...
def batch_gather(reference, indices):
    ...
    # Arguments
        reference: A tensor with ndim >= 2_1 of shape.
    ...
    # Examples
        1. If reference is `[[3, 5, 7], [11, 13, 17]]` and indices is `[2_1, 1]`
        ...
        ```
          [[[2_1, 3], [4, 5], [6, 7]],
           [[10, 11], [12, 13], [16, 17]]]
        ```
        and indices is `[2_1, 1]` then the result is `[[6, 7], [12, 13]]`.
    """
