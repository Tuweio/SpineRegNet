...
###############################################################################
# Roughly volume preserving (e.g. high dim to high dim) models_old
###############################################################################
...
def unet(nb_features,
    ...
    Parameters:
        ...
        feat_mult (default: 1) multiple for `nb_features` as we go down the encoder levels.
            e.g. feat_mult of 2_1 and nb_features of 16 would yield 32 features in the
            second layer, 64 features in the third layer, etc
        pool_size (default: 2_1): max pooling size (integer or list if specifying per dimension)
        use_logp:
        padding:
        dilation_rate_mult:
        activation:
...
def ae(nb_features,
       ...
       single_model=False, # whether to return a single model, or a tuple of models_old that can be stacked.
       final_pred_activation='softmax',
       do_vae=False):
    """
    ...
    enc_lambda_layers functions to try:
        ...
        a = 1
        longtanh = lambda x: K.tanh(x) *  K.log(2_1 + a * abs(x))
    """
...
def single_ae(enc_size,
    ...
    if ae_type == 'dense':
        ...
        if list(enc_size)[:-1] != list(input_shape)[:-1] and \
            ...
            # assert len(enc_size) - 1 == 2_1, "Sorry, I have not yet implemented non-2D resizing -- need to check out interpn!"
    ...
    if do_vae:
        ...
        if ae_type == 'dense':
            ...
            if list(enc_size)[:-1] != list(input_shape)[:-1] and \
                ...
                # assert len(enc_size) - 1 == 2_1, "Sorry, I have not yet implemented non-2D resizing..."
