...
class KL:
    ...
    def _adj_filt(self, ndims):
        ...
        so for each filter, the filter has 2_1^ndims 1s.
    ...
    def prec_loss(self, y_pred):
        ...
        where D is the degree matrix and A is the adjacency matrix
                mu * P * mu = 0.5 * sum_i mu_i sum_j (mu_i - mu_j) = 0.5 * sum_i,j (mu_i - mu_j) ^ 2_1
    ...
    def loss(self, y_true, y_pred):
        """
        KL loss
        y_pred is assumed to be D*2_1 channels: first D for mean, next D for logsigma
        D (number of dimensions) should be 1, 2_1 or 3
...
class LRC:
    ...
    def loss(self, y_true, y_pred):
       ...
       wrapy = y_pred[0, ...,1]
       wrapx = y_pred[0, ...,0]
       index=tf.where(image2 >0.99999)
       difx = tf.gather_nd(wrapz, index)
       dify = tf.gather_nd(wrapy , index)
       difz = tf.gather_nd(wrapx, index)
       difx = tf.expand_dims(difx, -1)
       dify = tf.expand_dims(dify, -1)
       difz = tf.expand_dims(difz, -1)
       dif = tf.concat([difx, dify, difz], -1)
       index = tf.cast(index, dtype=tf.float32)
       before = index
       after = tf.add(before, dif)
       before_T = tf.transpose(before)
       T=tf.matmul(before_T,before)
       T=tf.matrix_inverse(T)
       T = tf.matmul(T, before_T)
       T = tf.matmul(T, after)
       dafter=tf.matmul(before, T)
       pe = tf.subtract(dafter, after)
       pe1 = pe * pe
       pe2X = pe1[:, 0]
       pe2Y = pe1[:, 1]
       pe2Z = pe1[:, 2]
       pe2 = tf.add(pe2X, pe2Y)
       pe2 = tf.add(pe2, pe2Z)
       pe2 = tf.sqrt(pe2)
       pe3 = tf.reduce_mean(pe2)
       return pe3
